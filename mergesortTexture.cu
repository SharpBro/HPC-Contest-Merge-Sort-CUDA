...
texture<DATATYPE, 1> tx_sorted_ref; // TEXTURE creation
...
//  GPU Implementation                       //
// // // // // // // // // // // // // // // //
__device__ void merge_gpu(DATATYPE *list, DATATYPE *sorted, int start, int mid, int end, bool flag) {
...
__global__ void mergesort_gpu(DATATYPE *list, DATATYPE *sorted, int n, int chunk, bool flag) {
    ...
    merge_gpu(list, sorted, start, mid, end, flag);
}
...
int mergesort(DATATYPE *list, DATATYPE *sorted, int n) {
    ...
    for (chunk_size = 2; chunk_size < 2 * n; chunk_size *= 2) {
        ...
        if (sequential) {
            ...
            if (flag){
                mergesort_gpu<<<blocks_required, threads_per_block>>>(sorted_d, list_d, n, chunk_size, true);
            } else {
                mergesort_gpu<<<blocks_required, threads_per_block>>>(list_d, sorted_d, n, chunk_size, false);
            }
